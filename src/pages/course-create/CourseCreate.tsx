...
import {isDigitsOnly, isDigitsList, splitDigitsList, everyTokenIsNumber} from "../../utils/helpers-functions/course-create-helpers"
...
export default function CourseCreatePage() {
  ...
  /* Derived */
  const commanderPersonalNumbers = useMemo(() => splitDigitsList(commandersRaw), [commandersRaw]);
  const mmkNumberValid = isDigitsOnly(mmkNumber); 
  const courseNumberValid = isDigitsOnly(courseNumber);
  const commandersRawValid = isDigitsList(commandersRaw) && everyTokenIsNumber(commanderPersonalNumbers);
  const traineePnValid = isDigitsOnly(newTraineePersonalNumber);

  /* Mock for backend later */
  async function fetchTraineeByPersonalNumber(personalNumber: string): Promise<{ name: string | null }> {
    return { name: null };
  }
  ...
  const handleAddTrainee = async () => {
    ...
    if (!pn || !/^\d+$/.test(pn)) return; // חובה ספרות
    ...
    setTraineeList(prev => [...prev, { id: crypto.randomUUID(), personalNumber: pn, name }]);
    setNewTraineePersonalNumber("");
  };

  const handleRemoveTrainee = (id: string) => setTraineeList(prev => prev.filter(t => t.id !== id));

  const handleResetForm = () => {
    setCourseName(""); setCourseNumber(""); setCommandersRaw(""); setDepartmentName("");
    setTraineeList([]); setNewTraineePersonalNumber(""); setMmkNumber(""); 
  };

  const handleSubmitForm = (e: FormEvent) => {
    e.preventDefault();
    if (!courseNumberValid || !commandersRawValid || traineeList.some(t => !/^\d+$/.test(t.personalNumber))) {
    ...
    const payload = {
      ...
      trainees: traineeList.map(t => ({ personalNumber: t.personalNumber, name: t.name ?? null })),
    };
        ...
        <form className={styles.form} onSubmit={handleSubmitForm} dir="rtl">
          ...
          <div>
            <InputField
              ...
              height="44px"
              textSize="16px"
              onChange={(e: ChangeEvent<HTMLInputElement>) => setCourseNumber(e.target.value)}
            />
            {!courseNumberValid && (
          ...
          <div>
            <InputField
              ...
              height="44px"
              textSize="16px"
              onChange={(e: ChangeEvent<HTMLInputElement>) => setCommandersRaw(e.target.value)}
            />
            {!commandersRawValid && (
              <span className={styles.errorText}>מותר רק ספרות, פסיקים ורווחים.</span>
            )}

            {commanderPersonalNumbers.length > 0 && (
              <div className={styles.chipsRow}>
                {commanderPersonalNumbers.map((pn) => (
                  <span key={pn} className={styles.chip}>{pn}</span>
                ))}
              </div>
            )}
          </div>
          ...
          <div className={styles.addTraineeSection}>
            ...
            {!traineePnValid && (
              <span className={styles.errorText}>מספר אישי חייב להכיל ספרות בלבד.</span>
            )}
          </div>
              ...
              <table className={styles.table}>
                <tbody>
                  {traineeList.length === 0 ? (
                    <tr><td colSpan={3} className={styles.emptyCell}>אין חניכים בטבלה עדיין</td></tr>
                  ) : (
                    traineeList.map((t) => (
                      <tr key={t.id}>
                        <td className={styles.mono}>{t.personalNumber}</td>
                        <td className={styles.nameCell}>{t.name || <span className={styles.placeholder}>—</span>}</td>
                        <td>
                          <button type="button" className={styles.removeButton} onClick={() => handleRemoveTrainee(t.id)}>
                            הסר
                          </button>
                        </td>
                      </tr>
          ...
          <div className={styles.actions}>
            <button type="submit" className={styles.primaryButton}>שמור קורס</button>
            <button type="button" className={styles.secondaryButton} onClick={handleResetForm}>אפס טופס</button>
          </div>
        </form>
      </div>
    </div>
